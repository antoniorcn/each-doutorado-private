...
Exemplos de uso:
  ...
  python extract_hog_features.py --input "imagens/*.jpg" --output features.csv --wavelet-levels 3 --wavelet db4
    ...
    parser.add_argument(
        ...
        default=0,
        metavar='N',
        help='Número de níveis de decomposição wavelet a aplicar antes do HOG (padrão: 0, desabilitado). '
             'Cada nível reduz a imagem pela metade. Use valores como 1, 2, 3 para reduzir features.'
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='haar',
        help='Tipo de wavelet a ser usado (padrão: haar). Opções comuns: haar, db4, db8, bior2.2, coif2.'
    )
    ...
    if win_size[0] % cell_size[0] != 0 or win_size[1] % cell_size[1] != 0:
        print(f"Erro: win-size ({win_size}) deve ser múltiplo de cell-size ({cell_size})", file=sys.stderr)
        sys.exit(1)
    
    if block_size[0] % cell_size[0] != 0 or block_size[1] % cell_size[1] != 0:
        print(f"Erro: block-size ({block_size}) deve ser múltiplo de cell-size ({cell_size})", file=sys.stderr)
        sys.exit(1)
    
    if block_stride[0] % cell_size[0] != 0 or block_stride[1] % cell_size[1] != 0:
        print(f"Erro: block-stride ({block_stride}) deve ser múltiplo de cell-size ({cell_size})", file=sys.stderr)
        sys.exit(1)
    
    if block_size[0] > win_size[0] or block_size[1] > win_size[1]:
        print(f"Erro: block-size ({block_size}) não pode ser maior que win-size ({win_size})", file=sys.stderr)
        sys.exit(1)
    
    # Valida parâmetros do wavelet
    if args.wavelet_levels < 0:
        print(f"Erro: wavelet-levels deve ser >= 0 (recebido: {args.wavelet_levels})", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print("Tipos de wavelet válidos incluem: haar, db1-db20, bior2.2, bior4.4, coif1-coif5", file=sys.stderr)
        sys.exit(1)
    ...
    for image_path in image_files:
        ...
        if features is not None:
            if feature_size is None:
                ...
                print(f"Aviso: Tamanho de features inconsistente para {image_path}", file=sys.stderr)
    ...
    header: List[str] = ['image_path'] + [f'{FEATURE_COLUMN_PREFIX}{i}' for i in range(feature_size)]
