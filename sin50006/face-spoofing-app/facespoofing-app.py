...
class FaceDetectionApp(App):
    ...
    def build(self):
        ...
        self.fb_indicator = Label(text="Facebook FACEID", size_hint=(0.5, 1),
                                  color=(1, 1, 1, 1), bold=True)
        self.custom_indicator = Label(text="Modelo Customizado", size_hint=(0.5, 1),
                                      color=(1, 1, 1, 1), bold=True)
    ...
    def update(self, _):
        ...
        if ret:
            ...
            camera_buf = cv2.resize(frame, (380, 500)).tobytes()
            texture_camera = Texture.create(size=(380, 500), colorfmt='bgr')
            ...
            if faces_detected_qty > 0:
                ...
                display_frame = cv2.resize(face_img, (200, 200))
                buf = display_frame.tobytes()
                texture = Texture.create(size=(200, 200), colorfmt='bgr')
